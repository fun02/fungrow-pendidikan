...
<div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
    ...
    // =================================================================
    // 2. MODAL UTAMA DETAIL TUGAS (MODE POPUP MELAYANG & CATATAN HITAM)
    // =================================================================
    window.viewAssignmentDetail = async (courseId, asgId) => {
        try {
            ...
            // Simpan state global untuk modal Riwayat & Upload
                ...
                <div class="space-y-6 animate-fade">
                    <div class="space-y-3">
                        ...
                        <div class="grid grid-cols-[130px_1fr] gap-y-3 text-[11px]">
                            ...
                            <div class="font-medium text-slate-800 uppercase">${asg.title || 'Tugas'}</div>
                    ...
                    <div class="bg-blue-50 border border-blue-100 p-4 rounded-xl space-y-2">
                        ...
                        <p class="text-[11px] text-blue-700 whitespace-pre-line leading-relaxed">${asg.kelompok.anggota}</p>
                    </div>` : ''}
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="border border-slate-200 p-3 rounded-xl flex items-center justify-between bg-slate-50">
                            <div class="flex items-center gap-3 min-w-0">
                                ...
                                <div class="min-w-0">
                                    <p class="text-[11px] font-bold text-slate-800 truncate">Lampiran_Tugas</p>
                                    <p class="text-[9px] text-slate-500">Lampiran</p>
                                </div>
                            </div>
                            <button onclick="window.open('${asg.fileUrl}', '_blank')" class="text-[10px] font-bold text-blue-600 border border-blue-200 bg-white px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-blue-50 shrink-0">
            ...
            let riwayatSingkatHTML = mySubmissions.slice(0, 2).map((sub, index) => {
                ...
                return `
                <div class="relative pl-7 pb-4 group last:pb-0">
                    ${index === 0 && mySubmissions.length > 1 ? '<div class="absolute left-[11px] top-6 bottom-0 w-[2px] bg-slate-200"></div>' : ''}
                    <div class="absolute left-1 top-1 w-5 h-5 rounded-full ${isLatest ? 'bg-emerald-500' : 'bg-slate-400'} text-white flex items-center justify-center text-[9px] font-bold z-10 border-2 border-white">${mySubmissions.length - index}</div>
                    
                    <div class="bg-white border ${isLatest ? 'border-emerald-200 shadow-sm' : 'border-slate-200'} rounded-xl p-3">
                        <div class="flex items-center justify-between mb-1.5">
                            <div class="text-[9px] text-slate-500">${formatDate(sub.timestamp)}</div>
                            ${isLatest ? `<span class="text-[8px] bg-emerald-50 text-emerald-600 px-1.5 py-0.5 rounded font-bold border border-emerald-200">Versi Terakhir</span>` : ''}
                        </div>
                        <div class="flex items-center gap-2">
                            <i data-lucide="file-text" class="w-4 h-4 text-red-500 shrink-0"></i>
                            <p class="text-[10px] font-bold text-slate-800 truncate">${sub.fileName || 'Tugas.pdf'}</p>
                        </div>
                    </div>
                </div>`;
            }).join('');
                    ...
                    <div class="space-y-3 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div id="mhs-preview-container" class="hidden space-y-4 mt-2 animate-slide-up">
                            ...
                            <div>
                                ...
                                <textarea id="mhs-note" class="w-full border border-slate-300 rounded-xl p-3 text-xs text-black font-medium outline-none focus:border-blue-500 bg-white placeholder-slate-400 shadow-sm" placeholder="Tulis catatan revisi atau pesan..." rows="2"></textarea>
                            </div>
            ...
            // STRUKTUR MODAL DIKEMBALIKAN KE MODE MELAYANG (CENTERED POPUP)
            showGlobalModal(`
            <div class="bg-slate-50 w-full max-w-2xl mx-auto rounded-2xl sm:rounded-[2rem] flex flex-col z-[2000] shadow-2xl overflow-hidden animate-slide max-h-[90vh]">
                
                <header class="bg-white px-4 py-3 flex items-center justify-between border-b border-slate-200 shrink-0">
                    <div class="flex items-center gap-3">
                        <button onclick="closeGlobalModal()" class="p-2 -ml-2 rounded-full hover:bg-slate-100 text-slate-700 transition-colors active:scale-95"><i data-lucide="x" class="w-5 h-5"></i></button>
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative">
                            ...
                            <span class="absolute -top-0.5 right-0 w-2 h-2 bg-red-500 rounded-full"></span>
                        </div>
                    </div>
                </header>
                        ...
                        <div class="border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
                            <div class="flex items-center gap-3 p-3 bg-white border-b border-slate-100">
                                ...
                                <div class="min-w-0">
                                    ...
                                    <p class="text-[11px] sm:text-xs font-bold text-slate-800 mt-0.5 truncate">${asg.dosen || '-'}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 p-3 bg-white border-b border-slate-100">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] sm:text-xs font-bold text-slate-800 mt-0.5">${typeof formatDate === 'function' ? formatDate(asg.deadline) : '-'}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-3 p-3 bg-slate-50">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] sm:text-xs font-black ${sisaWaktuWarna} mt-0.5">${sisaWaktuTeks}</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    // =================================================================
    // 3. FUNGSI UPLOAD TUGAS MAHASISWA & PENANGANAN ERROR
    // =================================================================
    ...
    window.handleMhsUpload = async function(courseId, asgId) {
        ...
        try {
            // Tahap 1: Upload ke Server File (Cloudinary)
            let url = "";
            try {
            ...
            // Tahap 2: Simpan Database ke Firebase Firestore
            ...
            // MENANGKAP ERROR FIREBASE ATAU CLOUDINARY
            errorBox.innerHTML = `Gagal mengunggah file ke database.<br><span class="text-[9px] font-normal opacity-80">Alasan: ${err.message}</span><br><br><span class="text-[9px] font-bold mt-1 block text-orange-600">Pastikan Rules Firestore Anda sudah ditambahkan untuk submissions.</span>`;
    ...
    // =================================================================
    // 4. MODAL HALAMAN PENUH: RIWAYAT PENGUMPULAN
    // =================================================================
    window.viewRiwayatPengumpulan = function() {
        const mySubmissions = window.currentMySubmissions || [];
        const asg = window.currentAssignmentInfo || {};
        
        let timelineHTML = mySubmissions.map((sub, index) => {
            const isLatest = index === 0;
            const num = mySubmissions.length - index;
            return `
            <div class="relative pl-12 pb-6 group last:pb-0 animate-fade" style="animation-delay: ${index * 0.1}s">
                ${index !== mySubmissions.length - 1 ? `<div class="absolute left-[23px] top-8 bottom-[-8px] w-[2px] bg-slate-200"></div>` : ''}
                <div class="absolute left-2 top-0 w-8 h-8 rounded-full ${isLatest ? 'bg-emerald-500' : 'bg-slate-400'} text-white flex items-center justify-center text-[12px] font-bold z-10 border-4 border-white shadow-sm">${num}</div>
                
                <div class="bg-white border ${isLatest ? 'border-emerald-200 shadow-md' : 'border-slate-200 shadow-sm'} rounded-xl p-4 transition-all">
                    <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
                        <div class="flex items-center gap-2 text-[10px] sm:text-[11px] font-bold text-slate-600"><i data-lucide="calendar" class="w-3.5 h-3.5 text-slate-400"></i> ${formatDate(sub.timestamp)}</div>
                        ${isLatest ? `<span class="text-[9px] bg-emerald-50 text-emerald-600 px-2 py-1 rounded font-bold border border-emerald-200">Versi Terakhir (Dinilai)</span>` : ''}
                    </div>
                    
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 bg-red-100 text-red-500 rounded-lg flex items-center justify-center shrink-0"><i data-lucide="file-text" class="w-5 h-5"></i></div>
                        <div class="min-w-0 flex-1">
                            <p class="text-[11px] sm:text-xs font-bold text-slate-800 truncate">${sub.fileName || 'Tugas.pdf'}</p>
                            <p class="text-[10px] text-slate-500 mt-0.5">File • Diunggah</p>
                        </div>
                    </div>
                    
                    <div class="flex gap-2 justify-end mb-3 pb-3 border-b border-slate-100">
                        <a href="${sub.fileUrl}" target="_blank" class="px-4 py-2 bg-white border border-blue-200 text-blue-600 text-[10px] font-bold rounded-lg flex items-center gap-1.5 hover:bg-blue-50 transition-colors">
                            <i data-lucide="eye" class="w-3.5 h-3.5"></i> Lihat
                        </a>
                        <a href="${sub.fileUrl}" target="_blank" download class="px-3 py-2 bg-white border border-blue-200 text-blue-600 text-[10px] font-bold rounded-lg hover:bg-blue-50 transition-colors">
                            <i data-lucide="download" class="w-3.5 h-3.5"></i>
                        </a>
                    </div>
                    
                    <div class="flex items-center gap-2 text-[10px] font-bold ${isLatest ? 'text-emerald-600' : 'text-emerald-600 opacity-70'} mb-2">
                        <i data-lucide="check-circle-2" class="w-3.5 h-3.5"></i> Terkumpul tepat waktu
                    </div>
                    
                    ${isLatest ? `
                    <div class="bg-emerald-50/50 p-2.5 rounded-lg border border-emerald-100 text-[10px] text-emerald-800 leading-relaxed">
                        Ini adalah versi terakhir yang akan dinilai dosen.
                    </div>` : `
        ...
        showGlobalModal(`
        <div class="bg-slate-50 w-full max-w-xl mx-auto rounded-2xl sm:rounded-[1.5rem] flex flex-col z-[2000] shadow-2xl overflow-hidden animate-slide max-h-[90vh]">
            <header class="bg-white px-4 py-3 flex items-center justify-between border-b border-slate-200 shrink-0">
                <div class="flex items-center gap-3">
                    ...
                    <h2 class="font-bold text-slate-800 text-sm">Riwayat Pengumpulan</h2>
                </div>
                <button class="p-2 text-blue-500 hover:bg-blue-50 rounded-full"><i data-lucide="info" class="w-5 h-5"></i></button>
            </header>
            
            <div class="flex-1 overflow-y-auto hide-scrollbar p-5 space-y-6">
                <div class="bg-blue-50 border border-blue-200 p-4 rounded-xl flex items-start gap-3 shadow-sm">
                    <i data-lucide="info" class="w-5 h-5 text-blue-600 shrink-0"></i>
                    <p class="text-[11px] text-blue-800 leading-relaxed">Riwayat menampilkan semua versi file yang pernah Anda kumpulkan. Versi terakhir adalah yang akan dinilai oleh dosen.</p>
                </div>
                ...
                <div class="bg-amber-50 border border-amber-200 p-4 rounded-xl shadow-sm mt-4">
                    ...
                    <ul class="list-disc list-inside text-[11px] text-amber-800 space-y-1 ml-1">
                        <li>Anda masih bisa mengganti file sebelum deadline.</li>
                        <li>Setelah deadline, file tidak dapat diubah.</li>
                    </ul>
                </div>
                
                <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
                    <h3 class="font-bold text-slate-800 text-xs mb-3">Ringkasan Pengumpulan</h3>
                    <div class="space-y-2.5 text-[11px]">
                        <div class="flex justify-between"><span class="text-slate-500">Jumlah Pengumpulan</span><span class="font-bold text-slate-800">${mySubmissions.length} Kali</span></div>
                        <div class="flex justify-between"><span class="text-slate-500">Pengumpulan Terakhir</span><span class="font-bold text-slate-800">${mySubmissions.length > 0 ? formatDate(mySubmissions[0].timestamp) : '-'}</span></div>
                        <div class="flex justify-between"><span class="text-slate-500">Status</span><span class="font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">Terkumpul Tepat Waktu</span></div>
                        <div class="flex justify-between"><span class="text-slate-500">Deadline</span><span class="font-bold text-slate-800">${typeof formatDate === 'function' ? formatDate(asg.deadline) : '-'}</span></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    window.openAssignForm = async function() {
        ...
        try {
            ...
            <div class="bg-white w-full max-w-md mx-auto rounded-none sm:rounded-2xl overflow-hidden shadow-[0_25px_50px_-12px_rgba(11,29,58,0.25)] relative flex flex-col h-max max-h-[90vh]">
                ...
                <div class="bg-[#0B1D3A] px-8 py-6 text-center shrink-0 relative overflow-hidden">
                    <div class="absolute -right-4 -top-4 w-24 h-24 bg-white/5 rounded-full blur-2xl"></div>
                    <h2 class="text-white text-xl font-black tracking-widest relative z-10">BUAT TUGAS BARU</h2>
                    <p class="text-slate-400 text-[10px] mt-1 tracking-[0.2em] font-bold relative z-10">SISTEM MANAJEMEN PEMBELAJARAN</p>
                </div>

                <div class="p-6 md:p-8 overflow-y-auto hide-scrollbar space-y-6 bg-slate-50 flex-1">
                    
                    <div>
                        <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                            <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="book-marked" class="w-3.5 h-3.5"></i></div> PILIH MATA KULIAH
                        </label>
                        <div class="relative group">
                            <select id="asg-course-id" class="w-full bg-white border-2 border-slate-200 text-black font-bold rounded-xl p-3.5 text-sm outline-none focus:border-[#0B1D3A] transition-all appearance-none cursor-pointer shadow-sm">
                                ${courseOptionsHTML}
                            </select>
                            <i data-lucide="chevron-down" class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none group-focus-within:text-[#0B1D3A]"></i>
                        </div>
                    </div>

                    <div>
                        <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                            <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="layers" class="w-3.5 h-3.5"></i></div> JENIS TUGAS
                        </label>
                        <div class="relative group">
                            <select id="asg-jenis" onchange="window.toggleJenisLainnya(this.value)" class="w-full bg-white border-2 border-slate-200 text-black font-bold rounded-xl p-3.5 text-sm outline-none focus:border-[#0B1D3A] transition-all appearance-none cursor-pointer shadow-sm">
                                <option value="" disabled selected hidden>Tentukan jenis tugas...</option>
                                <option value="Makalah">Makalah Akademik</option>
                                <option value="Proposal">Proposal Proyek</option>
                                <option value="Karya Ilmiah">Karya Tulis Ilmiah</option>
                                <option value="Tulis Tangan">Tugas Tulis Tangan</option>
                                <option value="Lainnya">Lainnya (Ketik Manual)</option>
                            </select>
                            <i data-lucide="chevron-down" class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"></i>
                        </div>
                        <input type="text" id="asg-jenis-lainnya" class="hidden w-full bg-white border-2 border-[#0B1D3A] rounded-xl p-3.5 text-sm text-black font-bold outline-none shadow-md mt-3 animate-slide" placeholder="Contoh: Laporan Praktikum">
                    </div>

                    <div class="grid grid-cols-1 gap-4">
                        <div>
                            <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                                <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="users-2" class="w-3.5 h-3.5"></i></div> TARGET PENGERJAAN
                            </label>
                            <div class="relative">
                                <select id="asg-type" onchange="window.toggleGroupSection(this.value)" class="w-full bg-white border-2 border-slate-200 text-black font-bold rounded-xl p-3.5 text-sm outline-none focus:border-[#0B1D3A] appearance-none cursor-pointer">
                                    <option value="individu">Tugas Individu (Mandiri)</option>
                                    <option value="kelompok">Tugas Kelompok (Tim)</option>
                                </select>
                                <i data-lucide="chevron-down" class="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 pointer-events-none"></i>
                            </div>
                        </div>
                    ...
                    <div id="group-section" class="hidden mt-2 p-5 border-2 border-dashed border-[#0B1D3A]/20 bg-blue-50/50 rounded-2xl space-y-4 animate-slide">
                        <div>
                            <label class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Nama Identitas Kelompok</label>
                            <input type="text" id="asg-group-name" class="w-full bg-white border border-slate-300 rounded-lg p-3 text-xs text-black font-bold outline-none focus:border-[#0B1D3A]" value="Kelompok 1">
                        </div>
                        <div>
                            <label class="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1.5 block">Daftar Anggota Kelompok</label>
                            <textarea id="asg-group-members" class="w-full bg-white border border-slate-300 rounded-lg p-3 text-xs text-black font-bold outline-none focus:border-[#0B1D3A] h-20 resize-none leading-relaxed"></textarea>
                        </div>
                    </div>

                    <div>
                        <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                            <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="align-left" class="w-3.5 h-3.5"></i></div> INSTRUKSI PENGERJAAN
                        </label>
                        <textarea id="asg-desc" class="w-full bg-white border-2 border-slate-200 text-black font-medium rounded-xl p-4 text-sm outline-none focus:border-[#0B1D3A] h-28 resize-none shadow-sm transition-all" placeholder="Tuliskan poin-poin instruksi tugas di sini..."></textarea>
                    </div>

                    <div class="grid grid-cols-1 gap-6">
                        <div>
                            <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                                <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="calendar-days" class="w-3.5 h-3.5"></i></div> TENGGAT WAKTU (DEADLINE)
                            </label>
                            <input type="datetime-local" id="asg-deadline" class="w-full bg-white border-2 border-slate-200 text-black font-bold rounded-xl p-3.5 text-sm outline-none focus:border-[#0B1D3A] shadow-sm uppercase">
                        </div>

                        <div>
                            <label class="flex items-center gap-2 text-[10px] font-black text-[#0B1D3A] uppercase tracking-widest mb-2">
                                <div class="w-6 h-6 rounded-lg bg-[#0B1D3A] text-white flex items-center justify-center shrink-0 shadow-lg"><i data-lucide="paperclip" class="w-3.5 h-3.5"></i></div> LAMPIRAN MATERI (OPSIONAL)
                            </label>
                            <div onclick="document.getElementById('asg-file').click()" id="asg-upload-box" class="border-2 border-dashed border-slate-300 rounded-2xl p-8 text-center cursor-pointer hover:bg-slate-100 hover:border-[#0B1D3A] transition-all bg-white shadow-sm">
                                <i data-lucide="cloud-upload" id="asg-upload-icon" class="w-8 h-8 text-[#0B1D3A] mx-auto mb-2 opacity-50"></i>
                                <p id="asg-upload-text" class="text-[11px] font-black text-slate-700 uppercase tracking-tighter">Pilih File Referensi</p>
                                <p class="text-[9px] text-slate-400 mt-1 font-bold">PDF, DOCX, JPG (Maks. 20 MB)</p>
                            </div>
                            <input type="file" id="asg-file" class="hidden" onchange="window.handleAsgFileUpload(event)">
                        </div>
                    </div>
                ...
                <div class="p-6 md:p-8 bg-white border-t border-slate-200 shrink-0 flex flex-col gap-3">
                    <button onclick="window.submitNewAssignment()" id="btn-submit-asg" class="w-full py-4 bg-[#0B1D3A] hover:bg-black text-white rounded-xl font-black text-[12px] tracking-[0.2em] uppercase flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-xl shadow-blue-900/20">
                        <i data-lucide="rocket" class="w-4 h-4"></i> PUBLIKASIKAN TUGAS
                    </button>
                    <button onclick="closeGlobalModal()" class="w-full py-3 text-slate-400 hover:text-red-500 font-bold text-[10px] uppercase tracking-widest transition-colors">
                        Batalkan Sesi
                    </button>
                </div>
    ...
    // =====================================================================
    // 2. FUNGSI KIRIM KE DATABASE (DIPERKUAT)
    // =====================================================================
    window.submitNewAssignment = async function() {
        ...
        if (jenis === 'Lainnya') {
            ...
            if (!jenis) return alert('Ketikkan jenis tugas manual Anda!');
        } else if (!jenis) return alert('Harap tentukan Jenis Tugas!');

        if(!deadlineRaw) return alert('Harap tentukan Batas Waktu!');

        const btn = document.getElementById('btn-submit-asg');
        btn.innerHTML = '<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> PROSES PUBLIKASI...';
        btn.disabled = true;
        ...
        try {
            ...
            alert('Sukses! Tugas telah dipublikasikan.');
            STATE.asgPendingFile = null; 
            closeGlobalModal();
            ...
            btn.innerHTML = '<i data-lucide="rocket" class="w-4 h-4"></i> PUBLIKASIKAN TUGAS';
            btn.disabled = false;
        }
    };

...
<script>
    ...
    // ==============================================
    // 1. DASHBOARD LAYOUT & ROUTER (ANTI BLANK SCREEN)
    // ==============================================
    window.renderDashboardLayout = function(el) {
        try {
            ...
            const photo = STATE.currentUser?.photoURL ? 
                ...
                `<div class="w-10 h-10 rounded-full flex items-center justify-center font-bold bg-[#2563eb] text-white border-2 border-[#2563eb] shadow-sm">${STATE.currentUser?.displayName?.[0]?.toUpperCase() || 'U'}</div>`;
                ...
                <div class="flex flex-col h-full bg-[color:var(--bg)] text-[color:var(--text)] overflow-hidden relative">
                    <header class="px-5 py-4 flex justify-between items-center shrink-0 bg-[color:var(--surface)] border-b border-[color:var(--border)] relative z-30 shadow-sm backdrop-blur-xl">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <button onclick="if(typeof handleAISummary === 'function') handleAISummary(); else alert('Fitur AI sedang disiapkan');" class="p-2.5 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-600 text-white shadow-lg active:scale-95 transition-all"><i data-lucide="sparkles" class="w-5 h-5"></i></button>
                        </div>
                    </header>
            ...
            el.innerHTML = `<div class="p-10 text-white bg-red-600 font-bold h-full">CRASH PADA LAYOUT: ${e.message}</div>`;
        }
    };
    ...
    window.renderDashboardContent = function() {
        ...
        try {
            if (STATE.dashboardTab === 'home') {
                container.innerHTML = typeof getHomeHTML === 'function' ? getHomeHTML() : `<div class="p-6 bg-red-500 text-white">Fungsi getHomeHTML Hilang!</div>`;
            } 
            else if (STATE.dashboardTab === 'kelas') {
                container.innerHTML = typeof getKelasHTML === 'function' ? getKelasHTML() : `<div class="p-6 bg-red-500 text-white">Fungsi getKelasHTML Hilang!</div>`;
                ...
                container.innerHTML = typeof getJadwalHTML === 'function' ? getJadwalHTML() : `<div class="p-6 text-center italic opacity-50">Jadwal belum tersedia</div>`;
            } 
            ...
            else if (STATE.dashboardTab === 'about') container.innerHTML = typeof getAboutHTML === 'function' ? getAboutHTML() : '';
            
            if (typeof lucide !== 'undefined') lucide.createIcons();
        } catch(e) {
            container.innerHTML = `<div class="p-6 m-4 bg-red-100 text-red-600 border border-red-500 rounded-xl font-bold">Error Konten: ${e.message}</div>`;
        }
    };

    // ==============================================
    // 2. TUGAS KULIAH & TO-DO LIST (PRIBADI)
    // ==============================================
    window.renderAllAssignments = function() {
        try {
            ...
            const listHTML = allAsg.length === 0 ? `<div class="p-6 text-center border border-dashed border-[color:var(--border)] rounded-2xl"><p class="text-xs text-[color:var(--text2)] italic">Belum ada tugas kuliah.</p></div>` : allAsg.map(a => {
    ...
    window.renderSettings = function() {
        return `
            <div class="p-6 animate-fade space-y-6">
                <div><h2 class="text-xl font-black text-[color:var(--text)]">Pengaturan</h2><p class="text-[10px] text-[color:var(--text2)] uppercase font-bold tracking-widest">Akun & Preferensi Aplikasi</p></div>
                <div class="space-y-4">
                    <div class="glass p-5 rounded-3xl border border-[color:var(--border)] shadow-sm">
                        <div class="flex items-center gap-4 mb-5 pb-3 border-b border-[color:var(--border)]"><div class="w-10 h-10 rounded-xl bg-red-500/10 text-red-500 flex items-center justify-center border border-red-500/20 shadow-sm"><i data-lucide="shield-check" class="w-5 h-5"></i></div><div><h3 class="font-bold text-sm">Keamanan Akun</h3><p class="text-[9px] text-[color:var(--text2)] font-medium">Lindungi privasi dan akses Anda.</p></div></div>
                        <div class="space-y-2">
                            <button onclick="if(typeof openChangePasswordModal === 'function') openChangePasswordModal(); else alert('Fitur segera hadir!');" class="w-full p-3 rounded-xl bg-[color:var(--surface)] text-[11px] font-bold flex justify-between items-center border border-[color:var(--border)] active:scale-95 transition-all"><span class="flex items-center gap-3"><i data-lucide="key-round" class="w-4 h-4 text-amber-500"></i> Ubah Kata Sandi</span><i data-lucide="chevron-right" class="w-4 h-4 opacity-30"></i></button>
                        </div>
                    </div>
                    <div class="glass p-5 rounded-3xl border border-[color:var(--border)] shadow-sm">
                        <div class="flex items-center gap-4 mb-4"><div class="w-10 h-10 rounded-xl bg-indigo-500/10 text-indigo-500 flex items-center justify-center border border-indigo-500/20 shadow-sm"><i data-lucide="palette" class="w-5 h-5"></i></div><div><h3 class="font-bold text-sm">Tema Aplikasi</h3><p class="text-[9px] text-[color:var(--text2)] font-medium">Kustomisasi interface aplikasi.</p></div></div>
                        <button onclick="toggleTheme()" class="w-full p-4 rounded-2xl bg-gradient-to-r from-[#2563eb] to-indigo-600 text-white text-xs font-black flex justify-between items-center shadow-lg shadow-blue-500/20 active:scale-95 transition-all"><span>MODE ${STATE.isDark ? 'TERANG' : 'GELAP'}</span><i data-lucide="${STATE.isDark ? 'sun' : 'moon'}" class="w-5 h-5"></i></button>
                    </div>
                    <button onclick="if(auth) auth.signOut();" class="w-full py-4 rounded-3xl bg-red-500/5 text-red-500 font-black text-xs border border-red-500/20 hover:bg-red-500 hover:text-white transition-all shadow-sm">KELUAR DARI SISTEM</button>
                </div>
            </div>
        `;
    };
    ...
    // ==============================================
    // 4. DETAIL TUGAS & PENGUMPULAN (DESKTOP/MOBILE ADAPTIVE)
    // ==============================================
    ...
    window.updateAsgInstruksi = async function(courseId, asgId) { try { await db.collection('courses').doc(courseId).collection('assignments').doc(asgId).update({ description: document.getElementById('edit-asg-desc').value }); showToast("Instruksi berhasil disimpan!", "success"); } catch(e) { showToast("Gagal", "error"); } };
    window.handleMhsUpload = async function(e, courseId, asgId) { const file = e.target.files[0]; if(!file) return; if(file.size > 5 * 1024 * 1024) return alert("Gagal: Maksimal ukuran file adalah 5 MB!"); document.getElementById('mhs-upload-status').innerHTML = '<span class="text-xs text-blue-500 font-bold animate-pulse">Sedang mengunggah...</span>'; try { const url = await fetchCloudinaryUpload(file, false); await db.collection('courses').doc(courseId).collection('assignments').doc(asgId).collection('submissions').add({ userId: STATE.currentUser.uid, userName: STATE.currentUser.displayName, fileUrl: url, fileName: file.name, timestamp: firebase.firestore.FieldValue.serverTimestamp(), nilai: null }); document.getElementById('mhs-upload-status').innerHTML = '<span class="text-xs text-emerald-500 font-bold">Berhasil diunggah!</span>'; setTimeout(() => viewAssignmentDetail(courseId, asgId), 1000); } catch(err) { document.getElementById('mhs-upload-status').innerHTML = '<span class="text-xs text-red-500 font-bold">Gagal upload!</span>'; } };
    window.saveNilai = async function(courseId, asgId, subId) { const val = document.getElementById(`grade-${subId}`).value; if(!val) return; try { await db.collection('courses').doc(courseId).collection('assignments').doc(asgId).collection('submissions').doc(subId).update({ nilai: parseInt(val) }); showToast("Nilai berhasil disimpan!", "success"); } catch(e) { showToast("Gagal menyimpan nilai", "error"); } };
